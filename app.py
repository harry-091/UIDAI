...
st.caption(
    ...
    "along with short-term forecasts and analytical interpretations."
)
...
st.markdown(
    """
This dashboard analyzes Aadhaar system activity using **official datasets released through
government APIs**. The objective is to understand **how Aadhaar enrolment, demographic updates,
and biometric updates evolve over time**, and to provide **short-term operational forecasts**
...
st.markdown(
    """
The analysis is based on three primary datasets published through UIDAI and related
government data portals. These datasets capture **monthly Aadhaar activity** across
states, districts, and pincodes.
...
metric_label = st.selectbox("Metric", list(metric_label_map.keys()))
...
if forecast_df is not None:
    ...
    for insight in analyze_time_series(pd.concat([history[target], forecast_df[target]])):
        st.write("-", insight)
    ...
    if "explain" in forecast_df.columns and forecast_df["explain"].notna().any():
        ...
        for line in explain_shap_contributions(
            explain_df,
